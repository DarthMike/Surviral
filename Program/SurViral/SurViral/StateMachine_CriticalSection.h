/*
	Filename: StateMachine_CriticalSection.h
...
#ifndef _CRITICALSECTIONSTATEMACHINE
#define _CRITICALSECTIONSTATEMACHINE

//Library dependencies

//Class dependencies
#include "StateMachine.h"
#include "SM_CriticalSection_States.h"
#include "AIAgent.h"
#include "Shared_Resources.h"
#include "LogManager.h"

class StateMachine_CriticalSection : public StateMachine<AIAgent>
{
public:
	//----- CONSTRUCTORS/DESTRUCTORS -----
	StateMachine_CriticalSection(AIAgent* actor, PhysicsManagerPointer physicsptr, const std::string& type):
	  StateMachine(actor,physicsptr,type),
	  mPhysicsMgr(physicsptr),
	  mPlayerContacts(0)
	{
		assert(mSMType == "CriticalSection");
		//On start default to state "Wait"
		SetState("Wait");
	}
	virtual ~StateMachine_CriticalSection()
	{
		if(mCurrentState)
		{
