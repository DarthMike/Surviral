/*
	Filename: StateMachine_DataChunk.cpp
...
//Events plugin
bool StateMachine_DataChunk::HandleEvent(const EventData& data)			
{
...
bool StateMachine_DataChunk::HandleCollision(const CollisionEventData& data) 
{
	bool eventprocessed(false);
...
void StateMachine_DataChunk::_createState(const std::string& statename)  //Creates a state given a state name (string) - it is specific of every state machine
{
	if(statename == "Wait")
	{
		mCurrentState = new State_DataChunk_Wait(mActor,mPhysicsMgr);
	}
	else if(statename == "Break")
	{
		mCurrentState = new State_DataChunk_Break(mActor,mPhysicsMgr);
	}
	else if(statename == "Infect")
	{
		mCurrentState = new State_DataChunk_Infect(mActor,mPhysicsMgr);
	}//ELSE - Not defined state!
	else
	{
		SingletonLogMgr::Instance()->AddNewLine("StateMachine_DataChunk::_createState","State name '" + statename + "' for a MadMonkey is not correct! Program BUG!",LOGEXCEPTION);
	}//IF
