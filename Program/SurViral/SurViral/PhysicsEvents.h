...
public:
	...
	{}
	//----- GET/SET FUNCTIONS -----
	const OutOfLimitsData& GetEventData() const { return mData; } //Get const info
	//----- OTHER FUNCTIONS -----
...
//The "destroyed joint" event data
typedef struct DestroyedJointInfo
{
	DestroyedJointInfo(b2Joint* affected, const std::string& jointname):
	joint(affected),
	name(jointname)
	{}
	b2Joint* joint;
...
//Event class
class DestroyedJointEvent : public EventData
{
public:
	//----- CONSTRUCTORS/DESTRUCTORS -----
	DestroyedJointEvent(const GameEventType type, const DestroyedJointInfo& data, float timestamp = 0.0f):
	  EventData(type,timestamp),
	  mData(data)
	  {
		  //Assert event created with correct enabled types
		  assert (
			         mType == Event_DestroyedJoint
				  );
	  }
	~DestroyedJointEvent()
	{}
	//----- GET/SET FUNCTIONS -----
	const DestroyedJointInfo& GetEventData() const { return mData; } //Get const info
	//----- OTHER FUNCTIONS -----

protected:
	//----- INTERNAL VARIABLES -----
	DestroyedJointInfo mData; //Event data
	//----- INTERNAL FUNCTIONS -----
};
#endif