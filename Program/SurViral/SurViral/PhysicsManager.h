...
/*
	TODO:
	     -IM WONDERING IF IT IS NECESSARY TO KEEP TRACKING OF ALL JOINTS, SHAPES AND BODIES BY NAME. 
		  SHOULD REVIEW IT AND CHANGE IT MAYBE. IT IS POSSIBLE THAT THIS IS NOT NEEDED, AS IT ADDS A RUNTIME
		  COST (NOW I DONT SEE IT) AND INNECESSARY TRACKING OF NAME-OBJECT RELATIONSHIP, REALLY FOR NOTHING...
		  FOR NOW I ADDED DESTRUCTION OF ELEMENTS (JOINTS) AND CREATION (SHAPES) BY POINTER TOO, WITHOUT NAME.
*/
...
public:
	...
	void CreateCircleShape(b2CircleDef* definition, b2Body* body);
	void CreatePolygonShape(b2PolygonDef* definition, const std::string& bodyname);
	void CreatePolygonShape(b2PolygonDef* definition, b2Body* body);
