...
{	
	...
	else
	{
		...
		float refzoom (mGeneralCamPointer->GetZoom());
		...
		for(itr = mParallaxList.begin();itr != mParallaxList.end();++itr)
		{
			...
			//Apply a change of zoom as per layer config
			float zoomtoapply (refzoom * (*itr).relzoom);
			(*itr).thecamera->SetZoom(zoomtoapply);			
		}//LOOP END
	}
