...
int PhysicsManager::mBodiesCreated = 0;
...
b2Body *PhysicsManager::CreateBody(const b2BodyDef* definition,const std::string& name)
{
	...
	if(name == "")
		bodyname<<"_DYNAMICBODY_"<<mBodiesCreated;	
	else
		bodyname<<name;

	//Be sure body doesnt exist already
	PhysBodiesMapIterator itr = mBodiesMap.find(bodyname.str());
	...
	if(itr == mBodiesMap.end())
	{
		...
		//Add it to maps
		mBodiesMap.insert(std::pair<std::string,b2Body*>(bodyname.str(),newbody));
		++mBodiesCreated;
		return newbody;
	}
	else
...
void PhysicsManager::CreateCircleShape(b2CircleDef* definition, b2Body* body)
{
	assert(body);
	body->CreateShape(definition);
}
...
void PhysicsManager::CreateCircleShape(b2CircleDef* definition, const std::string& bodyname)
{
	...
	{
		//Add shape to body
		CreateCircleShape(definition,(*itr).second);
	}
	else
...
void PhysicsManager::CreatePolygonShape(b2PolygonDef* definition, b2Body* body)
{
	assert(body);
	body->CreateShape(definition);
}
...
void PhysicsManager::CreatePolygonShape(b2PolygonDef* definition, const std::string& bodyname)
{
	...
	{
		//Add shape to body
		CreatePolygonShape(definition,(*itr).second);
	}
	else
...
b2Joint* PhysicsManager::CreateFixedJoint(b2FixedJointDef* definition, const std::string& jointname)
{
	...
	if(jointitr != mJointsMap.end())
	{
		SingletonLogMgr::Instance()->AddNewLine("PhysicsManager::CreateFixedJoint","Error: intent to create the same joint twice: " + jointname ,LOGEXCEPTION);
		return NULL;
	}
	else //ELSE - Name of joint is correct
	{
...
//Destroy a joint by pointer
void PhysicsManager::DestroyJoint(b2Joint* joint)
{
	assert (joint);
	JointsMapIterator itr;

	//LOOP - Find it in Joints map (USELESS??? TODO)
	for(itr = mJointsMap.begin(); itr != mJointsMap.end(); ++itr)
	{
		if((*itr).second == joint)
		{
			mJointsMap.erase(itr);
